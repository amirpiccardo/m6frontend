...
const BlogPostDetail = ({ theme }) => {
    ...
    const fetchComments = async () => {
        try {
            const response = await fetch(`http://localhost:4040/blogPosts/${id}/comments`);
            const data = await response.json();
            setComments(data);
        } catch (error) {
    ...
    const handleDeleteComment = async (commentId) => {
        try {
            await fetch(`http://localhost:4040/blogPosts/${id}/comments/${commentId}`, {
                method: 'DELETE',
            });
    ...
    const handleUpdateComment = async () => {
        try {
            const response = await fetch(`http://localhost:4040/blogPosts/${id}/comments/${editingComment._id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <Container className="my-5">
            <Row>
                {/* Colonna di Sinistra - Dettagli del Post */}
                <Col lg={8} md={7} className="text-start">
                    <h1 className={theme === 'dark' ? 'text-light' : 'text-dark'}>{post.title}</h1>
                    ...
                    <img
                        ...
                        style={{ maxWidth: '100%' }}
                    />
                    <p className={`mx-auto ${theme === 'dark' ? 'text-light' : 'text-dark'}`} style={{
                        fontSize: '1.125rem',
                        lineHeight: '1.6',
                        textAlign: 'justify',
                    }}>
                ...
                <Col lg={4} md={5}>
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-3">
                        <Button variant="outline-secondary" onClick={() => setShowCommentForm(!showCommentForm)}>
