...
const SearchResults = ({ theme }) => {
  ...
  const fetchSearchResults = async (query, page) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4040/search?query=${query}&page=${page}&pageSize=${postsPerPage}`
      );
      if (!response.ok) throw new Error(`Errore: ${response.status}`);
      const data = await response.json();
      
      setResults(data.blogPosts || data); // Adatta alla risposta del backend
      setTotalPosts(data.total || data.length);  // Aggiungi il conteggio totale dei post per calcolare la paginazione
    } catch (error) {
  ...
  const handleLike = async (postId) => {
    try {
      const response = await fetch(`http://localhost:4040/blogPosts/${postId}/like`, {
        method: 'PUT',
      });

      if (!response.ok) throw new Error('Errore durante l\'incremento dei mi piace');
  ...
  const totalPages = Math.ceil(totalPosts / postsPerPage);  // Calcolo del numero totale di pagine
              ...
              <div className="card-body">
                ...
                <p className="card-text">
                  <small className="text-muted">
                    {post.author} - Tempo di Lettura: {post.readtime.value} {post.readtime.unit}
                  </small>
                </p>
                <Link 
